      ...
      <div className="h0full mt-4 pt-4 w-full flex-1 border-t border-gray-400 overflow-y-scroll">
        ...
        {queue.map((song) => (
          <QueueItem
            ...
            onPlay={() => {
              setMusic(song)
              setHistory((prev) => (music ? [music, ...prev] : prev))
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center space-x-1">
        <Tooltip id={music.id + 'playlisy'} place="top" noArrow opacity={1} />
        <Tooltip id={music.id + 'delete'} place="top" noArrow opacity={1} />
        {onPlaylistAdd && (
          <button
            className={`transition-colors ${
              inPlaylist ? 'text-[var(--primary)]' : 'text-white hover:text-[var(--primary)]'
            }`}
            disabled={inPlaylist}
            onClick={(e) => {
              e.stopPropagation()
          ...
          >
            {inPlaylist ? <ListCheck size={20} /> : <ListPlus size={20} />}
          </button>
        )}
        {onDelete && (
          <button
